...
def test_student_function(module_name, function_name, roll_number):
    ...
    test_images = [filename for filename in os.listdir(
        subdirectory_path) if filename.endswith("a.jpg")]
    try:
    ...
    for test_image_name in test_images:
        # try:
        test_image_path_a = os.path.join(os.path.join(
            os.getcwd(), 'ultimate_test'), test_image_name)
        base, ext = test_image_name.rsplit('.', 1)
        base = base[:len(base)-2]
        # Create the new filename with the updated suffix "b"
        new_filename = f"{base}_b.{ext}"
        test_image_path_b = os.path.join(os.path.join(
            os.getcwd(), 'ultimate_test'), new_filename)
...
if __name__ == "__main__":
    student_files = [filename for filename in os.listdir(
    ) if filename.endswith(".py") and filename != "tester3.py"]
        ...
        for student_file in student_files:
            try:
                ...
                if match:
                    ...
                    results = test_student_function(
                        student_file[:-3], "solution", roll_number)
