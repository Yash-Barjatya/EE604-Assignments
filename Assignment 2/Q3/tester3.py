...
def test_student_function(module_name, function_name, label_dict):
    ...
    test_images = [filename for filename in os.listdir(
        subdirectory_path) if filename.endswith(".jpg")]
    total_score = 0
    is_passed = False
    for test_image_name in test_images:
        try:
            test_image_path = os.path.join(os.path.join(
                os.getcwd(), 'test'), test_image_name)
            output = student_function(test_image_path)
            print(test_image_path, output)
...
if __name__ == "__main__":
    ...
    ) if filename.endswith(".py") and filename != "tester2.py"]
            ...
            try:
                ...
                if match:
                    ...
                    results = test_student_function(
                        student_file[:-3], "solution", label_dict)
